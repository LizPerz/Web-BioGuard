...
export function Reportes() {
  ...
  const [fecha, setFecha] = useState(aYYYYMMDD(new Date()));
  ...
  const periodosDisponibles = useMemo<Periodo[]>(() => {
    const opts: Periodo[] = ['dia', 'semana'];
    if (maxDias >= 15) opts.push('15d');
  ...
  useEffect(() => {
    if (!periodosDisponibles.includes(periodo)) setPeriodo('dia');
  }, [periodosDisponibles, periodo]);

  const calcularRango = (): { desde: Date; hasta: Date; texto: string } => {
    const hoy = hoyLocal();
    if (periodo === 'dia') {
      const [y, m, d] = fecha.split('-').map(Number);
      const desde = new Date(y, m - 1, d, 0, 0, 0, 0);
      const hasta = new Date(y, m - 1, d, 23, 59, 59, 999);
      return { desde, hasta, texto: formatearFecha(desde) };
    }
    const dias = periodo === 'semana' ? 7 : periodo === '15d' ? 15 : 30;
    const hasta = new Date();
    const desde = new Date(hoy.getTime() - (dias - 1) * MS_DIA);
    return { desde, hasta, texto: `${formatearFecha(desde)} — ${formatearFecha(hoy)}` };
  };
  ...
  const options = periodosDisponibles.map((p) => ({
    value: p,
    label: p === 'dia' ? 'Diario' : p === 'semana' ? 'Semana' : p === '15d' ? '15 días' : '30 días',
  }));
        ...
        <ContentCard className="reportes__controls-card">
          <div className="reportes__controls-row">
            <div className="reportes__period">
              <span className="reportes__controls-label">Período</span>
              <SegmentedControl options={options} value={periodo} onChange={(v) => setPeriodo(v as Periodo)} />
            </div>

            {periodo === 'dia' && (
              <div className="reportes__fecha">
                <span className="reportes__controls-label">Día específico</span>
          ...
          <p className="reportes__plan-hint">
            ...
            {maxDias >= 15 ? `, semanal y hasta ${maxDias} días de historial` : maxDias >= 7 ? ' y hasta 7 días de historial' : ''}.
          ...
          {metricas.totalLecturas > 0 && (
            <ContentCard className="reportes__tabla-card">
              <div className="reportes__card-title-row" style={{ marginBottom: 12 }}>
                ...
                <h3>{periodo === 'dia' ? 'Lecturas por hora' : 'Lecturas por día'}</h3>
