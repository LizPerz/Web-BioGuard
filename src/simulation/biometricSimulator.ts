...
export function generateLectura(): LecturaSensores {
  ...
  const estresPct = clamp(noise(estresBase, 8), 1, 100);

  let nivelRiesgo: LecturaSensores['nivelRiesgo'];
  if (estresPct >= 60) nivelRiesgo = 'Estres Alto';
  else if (estresPct >= 25) nivelRiesgo = 'Estres Moderado';
  else nivelRiesgo = 'Relajado';

  return {
    bpm: Math.round(bpm * 10) / 10,
    temperatura: Math.round(temperatura * 10) / 10,
    estresPct: Math.round(estresPct * 10) / 10,
    nivelRiesgo,
    timestamp: Date.now(),
  };
}
...
export function calculateRiesgoIA(lectura: LecturaSensores): number {
  ...
  const normEstres = (lectura.estresPct - 25) / 50;
  const normTemp = Math.abs(lectura.temperatura - 36.5) / 2;

  const z = -2.0 + 1.5 * normBpm + 1.0 * normEstres + 1.5 * normTemp;
